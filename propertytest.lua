--[[
               ...
               +++                                                                ++*#              
                     ...
                     bypass or crack the script for any reason is not allowed.
                      ...
                      Copyright © 2023-2026 Alchemy Hub. All rights reserved.
...
getgenv().add_score = function(num)
    local num = num or 1
    getgenv().score = getgenv().score + num
end

getgenv().total = 0
getgenv().add_total = function(num)
    local num = num or 1
...
local Requst_Test do
    ...
    local full_scripts = "https://raw.githubusercontent.com/x2neptunereal/Alchemy/main/gateway.luau"
    ...
    if success and (typeof(response) == 'table' and response.Success) then
        ...
        print("✅ Success ✅")
        print("| Connect With: HttpGet")
        print("| Connect Code: ".. response['Status Code'])
        print(string.format("| Scripts: %s", scripts))

        pcall(add_score, 10)

        local success, response = pcall(function()
            return (game:HttpGet(scripts))
        end)
    ...
    if success and (typeof(response) == 'table' and response.Success) then
        ...
        local success, response = pcall(function()
            return (request({Url = scripts, Method = "GET"}).Body)
        end)
        if success and (typeof(response) == 'string' and string.find(response, full_scripts)) then
            print("| Correctly: true")
            print("| Taken Time: ".. tostring(tick() - start) .."(seconds)")
...
local UNC_Test do
    ...
    task.defer(function()
        ...
        local outOf = tostring(passes) .. "/" .. tostring(passes + fails + undefined)

        pcall(add_score, passes)
...
repeat task.wait() until unc_success

local rate = math.round(score / total * 100)
